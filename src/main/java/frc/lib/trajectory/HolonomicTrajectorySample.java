...
/**
 * Immutable snapshot of a {@link HolonomicTrajectory} at a single instant — the minimum a follower
 * needs to close the loop (current setpoint pose + feed-forward chassis speeds).
 *
 * <p>Time {@code timestampSeconds} is measured from the start of the trajectory (t=0 is the initial
 * sample). Chassis speeds are field-relative for consistency with {@link
